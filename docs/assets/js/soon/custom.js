...
(function($) {
    ...
    **/
    var launch = new Date(2018, 10, 14, 11, 00);
    /**
    * The script
    **/
    var message = $('#message');
    ...
    function setDate(){
        ...
        if( launch < now ){
            ...
            message.html('POUR LE PLAISIR DES YEUX...');
        }
        else{
            var s = -now.getTimezoneOffset()*60 + (launch.getTime() - now.getTime())/1000;
            var d = Math.floor(s/86400);
            days.html('<h1>'+d+'</h1><p>Jours'+(d>1?'s':''),'</p>');
            s -= d*86400;

            var h = Math.floor(s/3600);
            hours.html('<h1>'+h+'</h1><p>Heures'+(h>1?'s':''),'</p>');
            s -= h*3600;
            ...
            s = Math.floor(s-m*60);
            seconds.html('<h1>'+s+'</h1><p>Secondes'+(s>1?'s':''),'</p>');
            setTimeout(setDate, 1000);

            message.html('MAIS PAS SI VITE QUE ÇA QUAND MÊME');
        }
    }
})(jQuery);
/******************************************************************************************************************************
